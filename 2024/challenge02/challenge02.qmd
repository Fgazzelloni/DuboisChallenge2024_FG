...
date: '2024-02-12'
image: 'https://raw.githubusercontent.com/ajstarks/dubois-data-portraits/master/challenge/2024/challenge02/original-plate-12.jpg'
image-alt: ''
description: ''
...
ch2_2%>%
  ...
  geom_ribbon(data=d2,mapping=aes(x=x,y=y,
                   ...
                   ymax=pmax(y, 5)), 
              fill='#dbc8b4')+
  #geom_line(data=d2,aes(x=x,y=y),color="black")+
  geom_line(data=d2%>%filter(x>=1 & x <= 9),aes(x=x,y=y),color="black")+
    geom_point(data=d2%>%filter(x>=1 & x <= 9),aes(x=x,y=y),color="black",size=0.1)+
  ...
  scale_x_reverse(name = "First Axis",
                  ...
                  sec.axis = sec_axis(~.*1, 
                                      ...
                                      breaks = 1:9,
                                      labels =years))+
  coord_flip(xlim = c(9,1),ylim = c(2.1,-0.8),
             clip = "off")+
...
```{r}
ggdraw()+
  draw_image("test_approx.png")
```


```{r}
ggsave("challenge2.png",bg="#dbc8b4",
       width = 5,height = 6)
```
